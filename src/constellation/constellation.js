(() => {
  // Kick the whole script only once both elements exist
  function waitForElements(callback, delay = 100) {
    const tryInit = () => {
      const canvas = document.getElementById('drawCanvas');
      const field  = document.getElementById('starfield');
      if (canvas && field) {
        callback(canvas, field);
      } else {
  ...
  // Main logic runs *only* after the DOM has both canvas + field
  waitForElements((canvas, field) => {
    /* ===== Setup =================================================== */
    ...
    const updatePrompt = () => {
      indicator.textContent = promptMode === 'click'
        ? 'Cliquez pour dessiner'
        : 'Double-cliquez pour effacer';
    };
    updatePrompt();
